...
function gerarCNPJAleatorio($mask=0) {
    ...
    // return $value <= 1 ? "Algo deu errado" : implode($gerNum);
    // return vsprintf("%s%s.%s%s%s.%s%s%s/%s%s%s%s-%s%s", $gerNum);
    return $mask==0 ?     	$value <= 1 ? "err-cnpj" : implode($gerNum):     	vsprintf("%s%s.%s%s%s.%s%s%s/%s%s%s%s-%s%s", $gerNum);
} 
